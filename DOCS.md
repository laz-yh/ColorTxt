...
│   ├── voiceRead/              # TTS Provider 注册与 synthesis IPC（edge / dashscope / minimax）
│   │   ├── providerRegistry.ts
│   │   ├── registerVoiceReadIpc.ts
│   │   └── providers/          # edgeProvider、dashscopeProvider、minimaxProvider
...
│       │   ├── voiceReadVoiceGroups.ts   # 各引擎音色下拉分组（Edge / 通义 / MiniMax 等）
│       │   └── defaultCacheDirs.ts       # 默认 AI 数据/模型/立绘缓存目录（与 preload 对齐）
└── shared/
    ├── packageDerived.ts           # 从 package 派生的共享元数据
    ├── voiceReadEngines.ts       # 引擎注册表（edge / system / dashscope / minimax）
    ├── voiceReadProfiles.ts      # 朗读方案、单/多音色设置、方案密钥槽
    ├── voiceReadEngineConfig.ts  # 各引擎 API 密钥与模型字段（通义 / MiniMax 分开）
    ...
    ├── voiceReadEmotion.ts       # 情绪映射（通义 / MiniMax 等）
    ├── voiceReadEdgeTtsVoices.ts / voiceReadDashscopeVoices.ts / voiceReadMinimaxModels.ts  # 音色与模型预设
    ...
    ├── apiEndpointPresets.ts       # 对话/文生图服务商预设（含 MiniMax、Agnes AI 等）、OpenAI 兼容 Base URL
...
- **`voiceReadSpeaker.ts`**：旁白/对白多音色模式下，批量调用对话模型识别引号对白说话人、性别与情绪（**`voiceRead:attributeSpeakers`** IPC；依赖 **AI 阅读助手** 已启用且配置对话端点）。
...
- **语音朗读**：**`useAppVoiceRead`** 驱动顶栏 **`VoiceReadToolbar`**；播放中 **`isVoiceReadNavigationBlocked`** 拦截侧栏 tab 切换与查找栏打开；设置 **`voiceRead.*`** 持久化于 **`colorTxt.ui.settings`**（见 **「语音朗读」**）。
...
- **`apiEndpointPresets.ts`**：对话 **`CHAT_API_PROVIDER_PRESETS`**（服务商名 + 官方 Base URL 两行下拉；含 **MiniMax**、**Agnes AI**、OpenRouter、Gemini OpenAI 兼容、**「自定义 OpenAI 兼容服务」** 等）；**`findChatProviderPresetByBaseUrl`** 与接口地址联动（手改地址可反推服务商，清空后保持「自定义」）。文生图 **`TXT2IMG_BACKEND_PRESETS`**（含 **MiniMax** `minimax_images`、**Agnes AI** `agnes_images`；服务商名 + 默认 Base URL 两行下拉；与 **`AITxt2ImgConfig.backend`** 一致）；选中服务商写入默认地址与默认云端模型，**不**按地址反推服务商。**`agnesApiLikely`** / **`minimaxApiLikely`** 供深度思考等按 URL 识别网关。语音朗读 / 万相文案统一为 **「阿里云通义（DashScope）」**（**`DASHSCOPE_PLATFORM_LABEL`** 等）；MiniMax 对话/文生图/ TTS 密钥在应用内**分开存储**（见 **「语音朗读」**、**「文生图服务商」**）。
...
| `AppConnectionTestButton.vue`                        | 设置页共用 **测试连接**（图标 pending/成功/失败；成功不弹框；配置指纹变更后重置）；用于 AI 阅读助手、向量模型、角色卡文生图、**语音朗读**（通义 / MiniMax） |
| `SettingsSkillsPanel.vue`                            | 「技能」：内置技能开关与覆盖、自定义技能列表；由父级 footer「添加技能」打开 **`SettingsSkillEditModal`**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| `SettingsVoiceReadPanel.vue`                         | 「语音朗读」：独立 **朗读方案** 区块（最多 12 套，含引擎、单/多音色、密钥）；**引擎**下拉（Edge TTS / 系统语音 / 通义 / MiniMax）；**朗读方案**（单音色 / 旁白·对白多音色）；多音色下 **对白引号样式**、**AI 识别说话人**（需 AI 总开关）；通义 / MiniMax **API 密钥** + **`AppConnectionTestButton`**（不出声/不出图）；模型建议（**`ApiEndpointInput`**）；语速/音调；试听预览。**密钥与 AI 对话/文生图分开存** |
| `VoiceReadToolbar.vue`                               | 顶栏朗读工具条：播放/暂停/停止、上一行/下一行、合成状态；播放中拦截侧栏跳转（见 **`useAppVoiceRead`**） |
| `AppCheckbox.vue`                                    | 设置页复选框（对白引号样式等多选） |
...
- **文生图**（本地 A1111 / ComfyUI 与云端图像 API）与 **语音朗读**（Edge TTS / 系统语音 / 通义 Qwen3-TTS / MiniMax TTS，见 **「语音朗读」**）为独立配置，亦不在上表；文生图预设见下节。
...
- **`colorTxt.ui.settings`**：**`aiSkillsEnabled`**、**`aiSkillOverrides`**、**`aiCustomSkills`**、**`aiAssistantDeepThinking`**、**`aiAssistantSpoilerSafe`**；**`voiceRead`**（引擎、朗读方案、单/多音色，见 **「语音朗读」**）；**`aiSmartFormat`**（编辑模式智能排版开关组，见 `@shared/aiSmartFormatTypes`）；**`textConvertZh`**、**`textConvertLetter`**、**`textConvertDigit`**（顶栏「转换」阅读模式展示层，见 `@shared/textConvertTypes`）；**`characterPortraitCacheDir`**（空串表示使用默认 `userData/CharacterPortrait`）；**`characterCardTextureEffect`**（角色卡闪卡纹理 id，默认 **`soft`**，见 **「角色卡 3D 倾斜与闪卡纹理」**）；**`wordcloudFontFamily`**、**`wordcloudAngleMode`**、**`wordcloudPaletteId`**（词云全屏 UI 偏好，见 **「词云图」**）。其余界面与阅读字段仍见「数据存储说明」中的 `PersistedSettingsData` / `cacheStore.ts`。
...
| `SettingsVoiceReadPanel.vue` | 「语音朗读」：朗读方案、引擎、单/多音色、AI 识别、通义/MiniMax 密钥与测试连接；见 **「语音朗读」** |
| `VoiceReadToolbar.vue` | 顶栏朗读控制条 |
| `SettingsSkillEditModal.vue` | 自定义技能新建/编辑弹窗 |
...
**多 TTS 引擎 Provider**、**多套朗读方案**、**单音色 / 旁白·对白多音色**、**AI 说话人识别** 与 **角色专属音色**；与 **「开发」** 章目录树、**`src/main/voiceRead/`**、**`useAppVoiceRead.ts`** 互为参照。

### 功能与入口

| 项目 | 说明 |
| ---- | ---- |
| 顶栏 | **`VoiceReadToolbar.vue`**：播放 / 暂停 / 停止、上一行 / 下一行、合成状态指示 |
...
通义 / MiniMax 设置页均提供 **`AppConnectionTestButton`**（仅校验密钥，**不合成**）。
...
| API 密钥 | 各方案 **`dashscopeApiKey` / `minimaxApiKey`** 加密存 **`SECRET_SLOT_VOICE_READ_PROFILE_KEYS`**（JSON）；活跃方案同步旧槽 **`voiceRead.dashscopeApiKey`**（**`registerSecretsIpc`**） |
...
| **AI 识别** | 多音色 + **AI 阅读助手已启用** 时可开 **`aiSpeakerRecognitionEnabled`**：朗读前主进程 **`attributeVoiceReadSpeakers`** 识别说话人姓名、性别、情绪；匹配角色 **`voiceReadVoiceId`** 或按男女声回退；消耗对话 Token（可展示在设置/侧栏） |

渲染侧 **`voiceReadLineBuild.ts`** 切段、**`voiceReadVoiceResolve.ts`** 解析每段 voiceId；**`voiceReadLinePlayer.ts`** 统一缓存与 IPC 合成入口。

### API 密钥分开存储（MiniMax / 通义）

同一 MiniMax 开放平台账号可复用，但应用内**分槽**保存，互不同步：
...
| **`voiceRead:listVoices`** | 动态音色列表（MiniMax 等） |
| **`voiceRead:healthCheck`** | 测试连接 |
| **`voiceRead:attributeSpeakers`** | AI 批量识别对白说话人（`ai/voiceReadSpeaker.ts`） |
...
| **`shared/voiceReadEmotion.ts`** | 情绪枚举与各引擎映射 |
